...
locals {
  ...
  query_function_arn = replace(
    element(
      concat(
        data.aws_lambda_function.find_snapshot.*.arn,
        [module.maintain-rds-final-snapshots-lambda.query_arn],
      ),
      0,
    ),
    ":$LATEST",
  ...
  query_function_role = element(
    split(
      "/",
      element(
        concat(
          data.aws_lambda_function.find_snapshot.*.role,
          [module.maintain-rds-final-snapshots-lambda.query_role],
        ),
        0,
      ),
    ),
...
locals {
  ...
  counter = format(
    "%05d",
    local.first_run ? 1 : replace(
      substr(
        format("%s%s", "00000", local.previous_final_snapshot),
        -5,
        -1,
      ),
      "/^0+(\\d+)/",
